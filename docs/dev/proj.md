...
1. Package publish authentication:
    ...
    1. Set the variable `PDM_PUBLISH_PASSWORD` with **Mask variable** flag, the password to authenticate to the repository (package index).
1. Renovate authentication:
    ...
    1. Navigate to the [CI/CD](https://gitlab.com/serious-scaffold/ss-python/-/settings/ci_cd) settings and the **Variables** section. Set the token as variable `RENOVATE_TOKEN` with **Make variable** flag.
